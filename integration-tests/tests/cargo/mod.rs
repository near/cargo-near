...
fn test_dependency_local_path() -> anyhow::Result<()> {
    let near_sdk_dir = clone_git_repo("1a0344fcd300d700601984deeeb84a7993c59f84")?;
...
fn test_dependency_local_path_with_version() -> anyhow::Result<()> {
    ...
    // near-sdk = { path = "::path::", version = "4.1.0-pre.0", features = ["abi"] }
    let abi_root = generate_abi_fn! {
...
fn test_dependency_no_default_features() -> anyhow::Result<()> {
    // near-sdk = { version = "4.1.0-pre.0", default-features = false, features = ["abi"] }
    let abi_root = generate_abi_fn! {
...
fn test_dependency_multiple_features() -> anyhow::Result<()> {
    // near-sdk = { version = "4.1.0-pre.0", features = ["abi", "unstable"] }
    let abi_root = generate_abi_fn! {
...
fn test_dependency_renamed() -> anyhow::Result<()> {
    // near = { version = "4.1.0-pre.0", package = "near-sdk", features = ["abi"] }
...
fn test_dependency_patch() -> anyhow::Result<()> {
    ...
    //
    // [patch.crates-io]
    // near-sdk = { git = "https://github.com/near/near-sdk-rs.git", rev = "1a0344fcd300d700601984deeeb84a7993c59f84" }
    let abi_root = generate_abi_fn! {
