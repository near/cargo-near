...
pub fn build(args: OptsExtended) -> Result<BuildArtifact<WASM>, Box<dyn std::error::Error>> {
...
impl<'a> OptsExtended<'a> {
    pub fn skip_or_compile(
        &self,
        version: &Version,
    ) -> Result<(BuildArtifact<WASM>, bool), Box<dyn std::error::Error>> {
    ...
    pub fn compile_near_artifact(&self) -> Result<BuildArtifact<WASM>, Box<dyn std::error::Error>> {
